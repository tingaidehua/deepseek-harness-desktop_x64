//! Harness Core 多版本与本地内核切换。
//!
//! 管理本地 CLI 内核与预打包的各个发布版本，支持切换活动内核、下载历史版本
//! 到槽位、卸载已下载的历史版本，以及通过用户包管理器 CLI 更新本地内核。

use crate::service::core;
use tauri::AppHandle;

/// 内核列表：本地 CLI 内核 + 预打包各版本（含 active 标记与 cli path）。
...
/// 切换活动内核（id: `local` | `app` | `app-<tag>`；持久化 + 预打包版本目录
/// 互换；自动重启由前端负责）
#[tauri::command]
...
/// 下载指定 tag 的预打包内核到历史槽位（不激活；切换由 `set_active_core` 完成）
#[tauri::command]
...
/// 通过用户包管理器 CLI 更新本地内核（npm `install -g @latest` /
