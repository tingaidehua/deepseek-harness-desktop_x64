...
pub fn build_cmd_shim(app_dir: &Path, dsh_home: &Path) -> String {
    let legacy_dsh_bin = app_dir.join("dependencies/dsh/node_modules/@deepseek-ai/dsh/lib/bin.js");
...
{user_dsh}
set "DSH_BIN={legacy_dsh_bin}"
set "ACTIVE_CORE_FILE=%APP_DIR%\dependencies\active-core"
if exist "%ACTIVE_CORE_FILE%" (
    set /p "ACTIVE_CORE_TAG="<"%ACTIVE_CORE_FILE%"
    if defined ACTIVE_CORE_TAG set "DSH_BIN=%APP_DIR%\dependencies\cores\%ACTIVE_CORE_TAG%\node_modules\@deepseek-ai\dsh\lib\bin.js"
)
if not exist "%DSH_BIN%" set "DSH_BIN={legacy_dsh_bin}"
...
"#,
        ...
        legacy_dsh_bin = escape_path_cmd(&legacy_dsh_bin),
...
$appDir = '{app_dir}'
$dshBin = '{legacy_dsh_bin}'
$activeCoreFile = Join-Path $appDir 'dependencies\active-core'
if (Test-Path -LiteralPath $activeCoreFile) {{
    $activeCoreTag = (Get-Content -LiteralPath $activeCoreFile -Raw).Trim()
    if ($activeCoreTag -match '^[A-Za-z0-9._-]+$') {{
        $selectedDshBin = Join-Path $appDir "dependencies\cores\$activeCoreTag\node_modules\@deepseek-ai\dsh\lib\bin.js"
        if (Test-Path -LiteralPath $selectedDshBin) {{ $dshBin = $selectedDshBin }}
    }}
}}
...
"#,
        ...
        legacy_dsh_bin = escape_path_ps1(&legacy_dsh_bin),
...
# Do not edit: regenerated by the desktop app on install/startup.
APP_DIR='{app_dir}'
DSH_BIN='{legacy_dsh_bin}'
ACTIVE_CORE_FILE="$APP_DIR/dependencies/active-core"
if [ -f "$ACTIVE_CORE_FILE" ]; then
  ACTIVE_CORE_TAG=$(tr -d '\r\n' < "$ACTIVE_CORE_FILE")
  case "$ACTIVE_CORE_TAG" in
    ''|*[!A-Za-z0-9._-]*) ;;
    *)
      SELECTED_DSH_BIN="$APP_DIR/dependencies/cores/$ACTIVE_CORE_TAG/node_modules/@deepseek-ai/dsh/lib/bin.js"
      if [ -f "$SELECTED_DSH_BIN" ]; then DSH_BIN="$SELECTED_DSH_BIN"; fi
      ;;
  esac
fi
{user_dsh}
...
"#,
        ...
        legacy_dsh_bin = escape_path_sh(&legacy_dsh_bin),
