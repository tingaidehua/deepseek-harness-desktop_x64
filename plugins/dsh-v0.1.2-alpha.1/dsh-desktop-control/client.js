window.__ModuleLoader__.load({
  ...
  factory: () => {
    ...
    async function startCurrentWorkspace(ctx, command) {
      ...
      ctx.get('uiWorkspace').startSession(before.currentWorkspaceId)
    ...
    async function connectCurrentWorkspace(ctx, command) {
      ...
      try {
        const sessionId = await ctx.get('uiWorkspace').connectWorkspace(before.currentWorkspaceId)
    ...
    async function clickArchiveCurrent(ctx, command) {
      ...
      const selected = [...document.querySelectorAll('[data-slot="sidebar"] [role="treeitem"][aria-selected="true"]')]
        .find(element => element.getClientRects().length > 0)
      const menuButton = selected === undefined
        ? undefined
        : selected.querySelector('button[aria-label]')
      ...
      const uiWorkspace = ctx.get('uiWorkspace')
      const originalArchiveSession = uiWorkspace.archiveSession
      let currentServiceCalls = 0
      let serviceError = null
      uiWorkspace.archiveSession = async function (...args) {
        currentServiceCalls += 1
        try {
          return await originalArchiveSession.apply(this, args)
        }
        catch (error) {
      ...
      try {
        menuButton.click()
        await new Promise(resolve => setTimeout(resolve, 100))
        archiveAction = visibleElementByText(new Set(['归档会话', 'Archive session']))
        ...
        uiWorkspace.archiveSession = originalArchiveSession
      }
      ...
      await report({ id: command.id, action: command.action, phase: 'failed', before, after: sessionState(ctx), currentServiceCalls, serviceError, error: 'ARCHIVE_STATE_TIMEOUT' })
    }
    ...
    return { apply, inject: ['sessions', 'workspaces', 'uiWorkspace'], name: 'dsh-desktop-control' }
  },
})
