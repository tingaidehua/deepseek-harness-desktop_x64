...
## [P1][反向] 验证同一内核热启动不重复安装插件
[测试类型] 性能、回归
[前置条件] 当前内核和 profile 已完成一次插件投影；8 个内置插件及用户选择的预设插件都存在；服务未运行
[测试步骤] 1. 启动服务并等待真实 Loader 功能面 ready。2. 停止并再次启动同一内核，读取 Desktop 启动日志。3. 检查 profile 依赖、`node_modules` 插件入口和第二轮启动图。4. 切换到另一受支持内核，再切回原内核并重复启动。
[预期结果] 1. 第二轮日志出现 `INTERNAL_PLUGIN_CACHE_HIT` 和 `MANAGED_PLUGIN_CACHE_HIT`，`STARTUP_PHASE` 给出各阶段耗时。2. 第二轮没有 `INTERNAL_PLUGIN_NEEDS_REINSTALL`、`MANAGED_PLUGIN_REBIND` 或插件安装子进程，但全部插件仍存在于 Loader 启动图并通过功能面矩阵。3. 跨内核切换时只为来源不匹配的插件执行必要重绑定；切回后的下一轮再次缓存命中。4. 轮询缩短只减少就绪发现延迟，不得把仅端口监听或 SPA fallback 当作 Loader ready。
