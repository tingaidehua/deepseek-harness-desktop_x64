...
## [P2] 验证服务使用当前活动内核的 bin.js 入口启动
[测试类型] 功能
[前置条件] 本机 PATH 命中 dsh（本地内核存在，lib/bin.js 可解析）；active_core 未显式设置（自动=本地优先）；依赖已安装；服务未运行
[测试步骤] 1. 确认活动内核来源 core::active_source 为 local。2. 点击「启动」。3. 对比服务进程命令行首参（bin.js）与本地内核路径
[预期结果] 1. 活动内核来源为 local（本地优先）。2. 状态进入 Running。3. 进程命令行首参为本地内核包目录下 lib/bin.js 的绝对路径，而非预打包 dependencies/dsh/node_modules/@deepseek-ai/dsh/lib/bin.js
