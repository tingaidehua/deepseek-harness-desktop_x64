...
7. `dsh-desktop-control` 通过带令牌的回环控制面调用 Desktop 权威 service，输出结构化轨迹、延迟和失败码。它随独立 DSH 进程运行；可在保留 Harness 的情况下受控退出 Desktop，壳崩溃后仍可读取落盘轨迹并恢复，恢复后的外壳接管原 Harness 进程。
