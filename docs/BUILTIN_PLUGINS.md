...
Current built-ins are `dsh-desktop-control`, `dsh-tauri`, `dsh-tauri-ui`, `dsh-tauri-worktree`, `dsh-tauri-panel`, `dsh-tauri-panel-extension`, `dsh-tauri-session`, and `dsh-tauri-rightclick`. The `dsh-desktop-control` Host entry runs in the independent Harness process, calls authoritative Desktop services through an authenticated loopback protocol, and retains diagnostics and shell recovery after the shell exits. Its Client entry joins the DSH client boot graph, performs real UI actions, and reports assertable results to the Host.
